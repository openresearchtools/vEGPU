...
PEGPU is the app-side Swift/AppKit runtime manager, UTM-derived embedded SPICE
GUI display client, ANGLE/CocoaSpice integration, model routing interface,
sidecar networking, and macOS orchestration layer. PEGPU Machine is a separate
app/repository for QEMU, VFIO, DriverKit, firmware, guest driver packages, and
their GPL/source bundles.

## License Boundary

PEGPU.app is distributed under the permissive Apache License, Version 2.0.
...
lives in the separate PEGPU Machine.app. The app bundles and loads components
...
`Contents/Resources/PEGPURoot/legal/generated`.

The app-visible `LICENSES` file covers the installed PEGPU.app
...
PEGPU Machine.app is distributed as the separate Machine/runtime application.
...
Apache-side PEGPU launcher/display/AI code stays separate.
...
its own bundle.
Those external files remain owned by PEGPU Machine.app:

- `/Applications/PEGPU Machine.app/Contents/Resources/ThirdPartyNotices/NOTICES`
- `/Applications/PEGPU Machine.app/Contents/Resources/ThirdPartyNotices/LICENSES`

## Guest VM Installation Notice

`legal/GUEST-VM-INSTALL-NOTICES.md` documents software that PEGPU installs
inside the Linux guest VM through Debian APT, local PEGPU/PEGPU Machine guest
...
PEGPU carries UTM/CocoaSpice modifications as patches under:
...
PEGPU modifies that routing model to route llama.cpp-compatible API requests
...
modified for PEGPU to support multiple models, runtime pairs, VM/macOS routing,
...
PEGPU does not store llama.cpp runtime binaries in this source repository.
Release packages bundle the latest llama.cpp ARM64 runtime build available at
PEGPU release time from openresearchtools/llama-cpp-arm64-builds. Additional
llama.cpp and TurboQuant runtime versions remain user-managed through /core.
Runtime artifacts carry their own license files. The PEGPU app source bundle
...
PEGPU can route to llama.cpp-family runtime builds based on Tom's TurboQuant
...
## PEGPU Linux Scaling Helper

The Linux scaling helper under `Resources/Guest/scaling-app` is original
OpenResearchTools/PEGPU code distributed under the MIT license. CI packages it as
`pegpu-scaling_*.deb` in a disposable Debian container. The package installs a
...
## PEGPU Linux Performance Helper

The Linux performance helper under `Resources/Guest/performance-app` is original
OpenResearchTools/PEGPU code distributed under the MIT license. It provides a
