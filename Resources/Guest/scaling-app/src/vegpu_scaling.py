...
def config_dir() -> Path:
    ...
    if base:
        return Path(base) / "vegpu-scaling"
    return Path.home() / ".config" / "vegpu-scaling"
...
def run_gui(args: argparse.Namespace) -> int:
    ...
    class ScalingWindow(Gtk.Window):
        def __init__(self) -> None:
            super().__init__(title="vEGPU Scaling")
            self.set_icon_name(APP_ICON_NAME)
            self.set_default_size(420, 300)
            self.set_resizable(False)
            self.connect("destroy", Gtk.main_quit)
            self.get_style_context().add_class("vegpu-window")
            ...
            title = Gtk.Label(label="vEGPU Scaling", xalign=0)
            title.get_style_context().add_class("vegpu-title")
            title_box.pack_start(title, False, False, 0)

            display_label = Gtk.Label(label=f"DISPLAY={require_display(args.display)}", xalign=0)
            display_label.get_style_context().add_class("vegpu-muted")
            title_box.pack_start(display_label, False, False, 0)

            card = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
            card.get_style_context().add_class("vegpu-scale-card")
            outer.pack_start(card, False, False, 0)

            scale_label = Gtk.Label(label="Scale", xalign=0)
            scale_label.get_style_context().add_class("vegpu-muted")
            ...
            for index, scale in enumerate(SUPPORTED_SCALES):
                ...
                button.get_style_context().add_class("vegpu-scale-button")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="vegpu-scaling", description="Sharp XFCE session scaling for vEGPU guests.")
