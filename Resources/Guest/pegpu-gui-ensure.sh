...
install_customization_script() {
  ...
  source_path="${PEGPU_CUSTOMIZATION_SOURCE:-$script_dir/customization.sh}"
  ...
  printf 'Missing PEGPU GUI customization script: %s\n' "$source_path" >&2
  exit 1
}
...
install_scaling_app() {
  ...
  script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
  source_dir="${PEGPU_SCALING_APP_SOURCE:-$script_dir/scaling-app}"
  package=""
...
install_desktop_mount_policy() {
  ...
  cat >/etc/polkit-1/rules.d/49-pegpu-desktop-mount.rules <<'EOS'
polkit.addRule(function(action, subject) {
  if (subject.user !== "pegpu") {
    return;
  }
  var allowed = [
...
EOS
  chmod 0644 /etc/polkit-1/rules.d/49-pegpu-desktop-mount.rules
...
configure_desktop_network() {
  ...
  cat >/etc/NetworkManager/conf.d/90-pegpu-managed.conf <<'EOF'
[ifupdown]
managed=true
...
EOF
  cat >/etc/NetworkManager/system-connections/pegpu-vmnet.nmconnection <<'EOF'
[connection]
id=PEGPU vmnet
...
method=disabled
EOF
  chmod 0600 /etc/NetworkManager/system-connections/pegpu-vmnet.nmconnection
  systemctl enable NetworkManager >/dev/null 2>&1 || true
  systemctl restart NetworkManager >/dev/null 2>&1 || true
  nmcli connection reload >/dev/null 2>&1 || true
  nmcli device set enp0s3 managed yes >/dev/null 2>&1 || true
  nmcli connection up "PEGPU vmnet" ifname enp0s3 >/dev/null 2>&1 || true
}
...
repair_desktop_links() {
  ...
  rm -f /usr/local/bin/pegpu-open-mac-share \
    "/home/$HUMAN_USER/Desktop/Mac Share.desktop" \
    "/home/$HUMAN_USER/.local/share/applications/pegpu-mac-share.desktop"
  ...
  app_file="/home/$HUMAN_USER/.local/share/applications/pegpu-mac-share.desktop"
...
install_display_control() {
  ...
  cat >/usr/local/sbin/pegpu-display-mode-helper <<'EOS'
#!/usr/bin/env bash
set -euo pipefail

MODE_DIR=/etc/pegpu
MODE_FILE="$MODE_DIR/display-mode.conf"
XORG_CONF=/etc/X11/xorg.conf.d/20-pegpu-virtio-display.conf
NO_IDLE_CONF=/etc/X11/xorg.conf.d/90-pegpu-no-idle.conf
CUSTOMIZATION_SCRIPT=/usr/local/libexec/pegpu/customization.sh
SESSION_ROOT=/run/pegpu-display-sessions
SESSION_STATE_ROOT=/var/lib/pegpu/display-sessions
...
Section "Device"
    Identifier "PEGPU Virtio Display"
    Driver "modesetting"
    BusID "$virtio_busid"
...
Section "Screen"
    Identifier "PEGPU Screen"
    Device "PEGPU Virtio Display"
EndSection

Section "ServerLayout"
    Identifier "PEGPU Layout"
    Screen "PEGPU Screen"
EndSection
CONF
}
...
Section "Device"
    Identifier "PEGPU External NVIDIA"
    Driver "nvidia"
    BusID "$nvidia_busid"
...
Section "Screen"
    Identifier "PEGPU External Screen"
    Device "PEGPU External NVIDIA"
EndSection

Section "ServerLayout"
    Identifier "PEGPU Layout"
    Screen "PEGPU External Screen"
EndSection
CONF
}
...
schedule_primary_apply_after_restart() {
  ...
  user="${PEGPU_DISPLAY_USER:-pegpu}"
  uid="$(id -u "$user" 2>/dev/null || true)"
  [ -n "$uid" ] || return 0
  if [ -n "${PEGPU_PRIMARY_DISPLAY:-}" ]; then
    display_args=(DISPLAY="$PEGPU_PRIMARY_DISPLAY")
  fi
  runuser_bin="$(command -v runuser || printf '%s\n' /usr/sbin/runuser)"
  systemctl stop pegpu-display-primary-apply.service >/dev/null 2>&1 || true
  systemctl reset-failed pegpu-display-primary-apply.service >/dev/null 2>&1 || true
  systemd-run --unit=pegpu-display-primary-apply --collect --on-active=8s \
    "$runuser_bin" -u "$user" -- \
      /usr/bin/env "${display_args[@]}" XAUTHORITY="/home/$user/.Xauthority" DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$uid/bus" PEGPU_DISPLAY_QUIET=1 \
      /usr/local/bin/pegpu-display-control --apply-primary-display >/dev/null 2>&1 || true
}
...
current_display_mode() {
  if [ -f "$MODE_FILE" ]; then
    awk -F= '$1 == "PEGPU_DISPLAY_MODE" { print substr($0, index($0, "=") + 1); exit }' "$MODE_FILE" 2>/dev/null || true
  fi
}
...
Section "Device"
    Identifier "PEGPU Native NVIDIA"
    Driver "nvidia"
    BusID "$busid"
    Option "AllowEmptyInitialConfiguration" "true"
    Option "PrimaryGPU" "true"
...
Section "Screen"
    Identifier "PEGPU Native NVIDIA Screen"
    Device "PEGPU Native NVIDIA"
EndSection

Section "ServerLayout"
    Identifier "PEGPU Native NVIDIA Layout"
    Screen 0 "PEGPU Native NVIDIA Screen" 0 0
EndSection
CONF
}
...
CONF
  ...
  # later monitor changes inside this session without PEGPU rewriting positions.
...
CONF
  ...
  external-primary)
    ...
    write_no_idle_flags
    write_mode_file <<CONF
PEGPU_DISPLAY_MODE=external-primary
PEGPU_NVIDIA_BDF=$nvidia_bdf
PEGPU_NVIDIA_INDEX=$nvidia_index
CONF
    schedule_primary_apply_after_restart
...
EOS
  chmod 0755 /usr/local/sbin/pegpu-display-mode-helper

  cat >/etc/systemd/system/pegpu-display-boot-reset.service <<'EOS'
[Unit]
Description=Reset PEGPU display mode to SPICE before the graphical login
After=local-fs.target
Before=display-manager.service lightdm.service

[Service]
Type=oneshot
ExecStart=/usr/local/sbin/pegpu-display-mode-helper boot-spice

[Install]
WantedBy=multi-user.target
EOS
  systemctl enable pegpu-display-boot-reset.service >/dev/null 2>&1 || true

cat >/usr/local/bin/pegpu-display-control <<'EOS'
#!/usr/bin/env bash
set -euo pipefail

MODE_FILE=/etc/pegpu/display-mode.conf
CUSTOMIZATION_SCRIPT=/usr/local/libexec/pegpu/customization.sh
...
status_json() {
  ...
  mode="$(mode_value PEGPU_DISPLAY_MODE)"
  [ -n "$mode" ] || mode=spice
  selected_bdf="$(mode_value PEGPU_NVIDIA_BDF)"
  if [ -n "$selected_bdf" ]; then
    selected_bdf="$(normalize_bdf "$selected_bdf" 2>/dev/null || printf '%s' "$selected_bdf")"
  fi
  selected_index="$(mode_value PEGPU_NVIDIA_INDEX)"
...
switch_external_primary() {
  ...
  bdf="$(normalize_bdf "${1:?missing NVIDIA PCI bus id}")"
  idx="${2:-}"
  sudo -n /usr/local/sbin/pegpu-display-mode-helper external-primary "$bdf" "$idx"
}

switch_spice() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper spice
}

reload_display() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper reload
}

sessions_json() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper sessions --json
}

session_start() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper session-start "${1:?missing NVIDIA PCI bus id}" "${2:-}"
}

session_enter() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper session-enter "${1:?missing session id}"
}

session_release() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper session-release
}

session_stop() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper session-stop "${1:?missing session id}"
}

session_reload() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper session-reload "${1:?missing session id}"
}

session_outputs() {
  sudo -n /usr/local/sbin/pegpu-display-mode-helper session-outputs "${1:?missing session id}"
}
  ...
  case "${1:-}" in
    ...
    status)
      if [ "${2:-}" = "--json" ]; then
        ...
        sudo -n /usr/local/sbin/pegpu-display-mode-helper status
      fi
      return 0
      ;;
    list-gpus)
...
main "$@"
EOS
  chmod 0755 /usr/local/bin/pegpu-display-control

  cat >/etc/sudoers.d/90-pegpu-display-control <<'EOS'
pegpu ALL=(root) NOPASSWD: /usr/local/sbin/pegpu-display-mode-helper *
EOS
  chmod 0440 /etc/sudoers.d/90-pegpu-display-control
  visudo -cf /etc/sudoers.d/90-pegpu-display-control >/dev/null 2>&1 || rm -f /etc/sudoers.d/90-pegpu-display-control

  find "/home/$HUMAN_USER/.config/autostart" -maxdepth 1 -type f -name 'pegpu-display-*.desktop' -delete
  cat >"/home/$HUMAN_USER/.config/autostart/pegpu-display-apply.desktop" <<'EOS'
[Desktop Entry]
Type=Application
Name=PEGPU Display Apply
Exec=/bin/sh -lc 'sleep 2; /usr/local/bin/pegpu-display-control --apply-primary-display'
OnlyShowIn=XFCE;
X-GNOME-Autostart-enabled=true
EOS
  chown "$HUMAN_USER:$HUMAN_USER" "/home/$HUMAN_USER/.config/autostart/pegpu-display-apply.desktop"
...
display_mode_value() {
  local key="$1"
  [ -f /etc/pegpu/display-mode.conf ] || return 0
  awk -F= -v key="$key" '$1 == key { print substr($0, index($0, "=") + 1); exit }' /etc/pegpu/display-mode.conf
}

force_spice_mode_on_launch() {
  [ "${PEGPU_FORCE_SPICE_ON_LAUNCH:-0}" = "1" ] || return 0
  case "$(display_mode_value PEGPU_DISPLAY_MODE)" in
    external-primary) return 0 ;;
  esac
  install -d /etc/pegpu
  cat >/etc/pegpu/display-mode.conf <<'CONF'
PEGPU_DISPLAY_MODE=spice
CONF
}
...
write_spice_only_xorg_display() {
  local busid="$1"
  write_root_file_if_changed /etc/X11/xorg.conf.d/20-pegpu-virtio-display.conf <<EOS
Section "ServerFlags"
    Option "AutoAddGPU" "false"
    Option "AutoBindGPU" "false"
EndSection
...
write_no_idle_xorg_display() {
  write_root_file_if_changed /etc/X11/xorg.conf.d/90-pegpu-no-idle.conf <<'EOS'
Section "ServerFlags"
    Option "AutoAddGPU" "false"
    Option "AutoBindGPU" "false"
    Option "BlankTime" "0"
...
configure_virtio_xorg_display() {
  ...
  busid="$(xorg_bus_id_from_bdf "$bdf")"
  mode="$(display_mode_value PEGPU_DISPLAY_MODE)"
  nvidia_bdf="$(display_mode_value PEGPU_NVIDIA_BDF)"
  case "$mode" in
    external-primary)
...
cat >/etc/lightdm/lightdm.conf.d/90-pegpu-autologin.conf <<'EOS'
[Seat:*]
autologin-user=pegpu
...
install -d -o pegpu -g pegpu \
  /home/pegpu/.config/xfce4/xfconf/xfce-perchannel-xml \
  /home/pegpu/.config/autostart

chown -R pegpu:pegpu /home/pegpu/.config
usermod -aG video,render,input,dialout pegpu || true
