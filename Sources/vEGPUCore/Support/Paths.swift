...
public struct AppPaths: Sendable {
    ...
    public init(root: URL = AppPaths.discoverRoot(), environment: [String: String] = ProcessInfo.processInfo.environment) {
        self.root = root
        let dataRoot = environment["VEGPU_APP_DATA_DIR"].flatMap { $0.isEmpty ? nil : URL(fileURLWithPath: ($0 as NSString).expandingTildeInPath) }
            ?? FileManager.default.homeDirectoryForCurrentUser
                .appendingPathComponent("Library/Application Support/vEGPU/Machine", isDirectory: true)
    ...
    public static func discoverRoot(startingAt start: URL = URL(fileURLWithPath: #filePath)) -> URL {
        if let bundledRoot = Bundle.main.resourceURL?.appendingPathComponent("vEGPURoot", isDirectory: true),
