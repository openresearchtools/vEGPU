...
public final class NativeBridgeService: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.vegpu.app.native-bridge")
    ...
    public func environment() throws -> [String: String] {
        let info = try start()
        return [
            "VEGPU_NATIVE_BRIDGE_URL": info.url,
            "VEGPU_NATIVE_BRIDGE_TOKEN": info.token
        ]
    }
...
private final class NativeBridgeConnection: @unchecked Sendable {
    ...
    private func handle(_ request: HTTPBridgeRequest) {
        ...
        let supplied = request.headers["x-vegpu-bridge-token"] ?? request.headers["authorization"]?.replacingOccurrences(of: "Bearer ", with: "")
