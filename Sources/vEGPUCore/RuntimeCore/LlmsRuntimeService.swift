...
public final class LlmsRuntimeService: @unchecked Sendable {
    ...
    private let childRunDir = "/var/tmp/vegpu-llms/run"
    private let childLogDir = "/var/tmp/vegpu-llms/log"
    ...
    private let guestRuntimeRoot = "/home/vegpu/custom-llama-runtimes"
    private let guestHFRoot = "/home/vegpu/.cache/huggingface/hub"
    private let guestLMStudioRoot = "/home/vegpu/.lmstudio/models"
    ...
    public func command(args: [String]) async throws -> String {
        ...
        case "runtime-installed":
            guard let id = value else { throw RuntimeError.message("usage: vegpu machine llms-runtime runtime-installed <runtime-id>") }
            return try encode(await runtimeInstalled(id: id))
        case "delete-runtime":
            guard let id = value else { throw RuntimeError.message("usage: vegpu machine llms-runtime delete-runtime <runtime-id>") }
        ...
        case "stop":
            guard let id = value else { throw RuntimeError.message("usage: vegpu machine llms-runtime stop <runtime-id>") }
            try await stopChild(id: id)
            return try encode(StopResponse(ok: true, id: id))
        case "tail-log":
            guard let id = value else { throw RuntimeError.message("usage: vegpu machine llms-runtime tail-log <runtime-id>") }
        ...
        default:
            return "usage: vegpu machine llms-runtime <ensure|install-runtime|runtime-installed|delete-runtime|start-server|start-rpc|list-devices|list-devices-if-running|list-models|list-models-if-running|download-hf|copy-model|tail-log|stop|stop-servers|status> [json-spec|runtime-id]\n"
        }
    }
    ...
    public func listModels() async throws -> [LlmsModelInfo] {
        ...
        let script = """
        sudo -n -u vegpu python3 - <<'PY'
        import hashlib, json, os, re

        roots = [
            ("huggingface", "/home/vegpu/.cache/huggingface/hub"),
            ("lmstudio", "/home/vegpu/.lmstudio/models"),
        ]
    ...
    public func downloadHF(spec: LlmsHFDownloadSpec) async throws {
        guard machine.currentPid() != nil else {
            throw RuntimeError.message("vEGPU runtime is not running. Start the runtime before downloading into the VM.")
        }
        ...
        let setupScript = [
            "set -eu",
            "sudo -n -u vegpu install -d -m 0755 \(shellQuote("\(guestHFRoot)/\(repoDir)/snapshots/\(revision)"))"
        ].joined(separator: "\n")
        ...
        for remotePath in paths {
            ...
            let script = [
                "set -eu",
                "sudo -n -u vegpu install -d -m 0755 \(shellQuote(URL(fileURLWithPath: target).deletingLastPathComponent().path))",
                "rm -f \(shellQuote(temp))",
                "sudo -n -u vegpu sh -lc \(shellQuote("curl -L --fail --retry 3 --connect-timeout 15 \(headerArgs)-o \(shellQuote(temp)) \(shellQuote(url))"))",
                "sudo -n -u vegpu mv \(shellQuote(temp)) \(shellQuote(target))"
            ].joined(separator: "\n")
    ...
    public func copyModel(spec: LlmsModelCopySpec) async throws -> LlmsModelCopyResult {
        ...
        guard machine.currentPid() != nil else {
            throw RuntimeError.message("vEGPU runtime is not running. Start the runtime before copying VM models.")
        }
        try await ssh.waitForSSH()
    ...
    private func copyMacModelToVM(files: [String], macRoot: String, vmRoot: String, progressPath: String?) async throws -> LlmsModelCopyResult {
        ...
        let remoteDir = "/var/tmp/vegpu-model-copy-\(token)"
        ...
        for (index, file) in files.enumerated() {
            ...
            let script = """
            set -eu
            sudo -n install -d -o vegpu -g vegpu -m 0755 \(shellQuote(URL(fileURLWithPath: destination).deletingLastPathComponent().path))
            sudo -n install -o vegpu -g vegpu -m 0644 \(shellQuote(remoteTemp)) \(shellQuote(destination))
    ...
    private func copyVMModelToMac(files: [String], macRoot: String, vmRoot: String, progressPath: String?) async throws -> LlmsModelCopyResult {
        ...
        var copied: [String] = []
        let localTempRoot = FileManager.default.temporaryDirectory.appendingPathComponent("vegpu-model-copy-\(token)", isDirectory: true)
        ...
        for (index, file) in files.enumerated() {
            ...
            let script = """
            set -eu
            sudo -n -u vegpu test -f \(shellQuote(file))
            sudo -n -u vegpu cp \(shellQuote(file)) \(shellQuote(remoteTemp))
    ...
    private func guestFileSizes(_ files: [String]) async throws -> [String: Int64] {
        var sizes: [String: Int64] = [:]
        for file in files {
            let out = try await runGuestScript("sudo -n -u vegpu stat -c %s \(shellQuote(file))", timeout: 10)
    ...
    public func ensureDirectRuntime() async throws {
        guard machine.currentPid() != nil else {
            throw RuntimeError.message("vEGPU runtime is not running. Start the runtime before using VM accelerator devices.")
        }
        try await ssh.waitForSSH()
        let script = """
        mkdir -p \(shellQuote(childRunDir)) \(shellQuote(childLogDir))
    ...
    public func installRuntime(spec: LlmsRuntimeInstallSpec) async throws -> LlmsRuntimeInstallResult {
        guard machine.currentPid() != nil else {
            throw RuntimeError.message("vEGPU runtime is not running. Start the runtime before installing a Linux llama runtime.")
        }
        ...
        let remoteArchive = "/var/tmp/vegpu-llms-runtime-\(id).tar.gz"
        ...
        let script = ([
            "set -eu",
            "sudo -n install -d -o vegpu -g vegpu -m 0755 \(shellQuote(guestRuntimeRoot))",
            "sudo -n rm -rf \(shellQuote(root)) \(shellQuote(root + ".tmp"))",
            "sudo -n install -d -o vegpu -g vegpu -m 0755 \(shellQuote(root + ".tmp"))",
            "sudo -n -u vegpu tar -xzf \(shellQuote(remoteArchive)) -C \(shellQuote(root + ".tmp"))",
    ...
    public func deleteInstalledRuntime(id: String) async throws {
        guard machine.currentPid() != nil else {
            throw RuntimeError.message("vEGPU runtime is not running. Start the runtime before deleting a Linux VM runtime.")
        }
        try await ssh.waitForSSH()
        let safe = safeName(id)
        let root = "\(guestRuntimeRoot)/\(safe)"
