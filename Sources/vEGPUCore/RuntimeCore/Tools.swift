...
public final class ToolResolver: @unchecked Sendable {
    ...
    public func resolve() throws -> ToolPaths {
        let qemuShare = environment["VEGPU_QEMU_SHARE"].flatMap { $0.isEmpty ? nil : $0 }
            ?? VfioApp.resourcesPath("share", "qemu", environment: environment)
        return ToolPaths(
            qemu: try executable(environment["VEGPU_QEMU"] ?? VfioApp.macOSPath("qemu-system-aarch64", environment: environment), hint: "qemu-system-aarch64"),
            qemuLauncher: try executable(VfioApp.helperPath(environment: environment), hint: "qemu-vfio-apple"),
            qemuImg: try executable(environment["VEGPU_QEMU_IMG"] ?? VfioApp.macOSPath("qemu-img", environment: environment), hint: "qemu-img"),
