...
public enum AngleRuntime {
    ...
    public static func candidateFrameworkDirectories(
        ...
        if let override = environment["VEGPU_ANGLE_FRAMEWORK_DIR"], !override.isEmpty {
            append(URL(fileURLWithPath: (override as NSString).expandingTildeInPath, isDirectory: true))
        }
        if let override = environment["VEGPU_DISPLAY_FRAMEWORKS_OUT"], !override.isEmpty {
            append(URL(fileURLWithPath: (override as NSString).expandingTildeInPath, isDirectory: true))
        }
        if let override = environment["VEGPU_DISPLAY_FRAMEWORKS_DIR"], !override.isEmpty {
            append(URL(fileURLWithPath: (override as NSString).expandingTildeInPath, isDirectory: true))
        }
