...
public final class HostSleepGuardService: @unchecked Sendable {
    public static let helperPath = "/usr/local/sbin/vegpu-host-sleep-guard"
    ...
    public func ensureInstalled() async throws {
        if FileManager.default.isExecutableFile(atPath: Self.helperPath),
           (try? String(contentsOfFile: Self.helperPath, encoding: .utf8).contains("VEGPU_SLEEP_GUARD_HELPER_VERSION=\(Self.helperVersion)")) == true {
        ...
        _ = try await runner.runAdminScript(installScript(user: NSUserName()), prompt: [
            "vEGPU needs your Mac password once to install a narrow root-owned sleep guard helper.",
            "PCIe passthrough can wedge or panic the Mac if macOS sleeps while the runtime is active.",
            "The helper can only start, stop, and report the vEGPU sleep guard."
        ].joined(separator: " "))
    }
    ...
    private func installScript(user: String) -> String {
        ...
        return #"""
        set -eu
        HELPER=/usr/local/sbin/vegpu-host-sleep-guard
        SUDOERS=/etc/sudoers.d/90-vegpu-host-sleep-guard
        install -d -m 0755 /usr/local/sbin /etc/sudoers.d
        cat >"$HELPER" <<'EOS'
        #!/usr/bin/env bash
        set -u

        STATE_DIR="/Library/Application Support/vEGPU/SleepGuard"
        VEGPU_SLEEP_GUARD_HELPER_VERSION=\#(Self.helperVersion)
        ...
        worker() {
          ...
          printf '%s\n' "$$" >"$WORKER_PID"
          rm -f "$COMMAND"
          write_status true "$mode" "$target_pid" "$method" "preventing Mac sleep while vEGPU runtime is active"
          while :; do
        ...
        chmod 0755 "$HELPER"
        cat >"$SUDOERS" <<'EOS'
        \#(sudoersUser) ALL=(root) NOPASSWD: /usr/local/sbin/vegpu-host-sleep-guard start *, /usr/local/sbin/vegpu-host-sleep-guard force-on, /usr/local/sbin/vegpu-host-sleep-guard stop, /usr/local/sbin/vegpu-host-sleep-guard status
        EOS
        chmod 0440 "$SUDOERS"
