...
public final class SSHClient: @unchecked Sendable {
    public static let user = "vegpu"
    public static let controlUser = "vegpuctl"
    ...
    public func ensureKey() async throws -> String {
        ...
        if !FileManager.default.fileExists(atPath: privateKeyPath) {
            _ = try await runner.runChecked("/usr/bin/ssh-keygen", ["-t", "ed25519", "-N", "", "-f", privateKeyPath, "-C", "vegpu-machine"])
        }
    ...
    public func agentCommand(_ args: [String]) -> String {
        "sudo -n /usr/local/libexec/vegpu/vegpu-agent \(args.map(shellQuote).joined(separator: " "))"
    }
    ...
    public func waitForSSH(timeout: TimeInterval = 180) async throws {
        ...
        if role == .control {
            ...
            if await humanSSH.sshMaybe("true") {
                throw RuntimeError.message("Runtime SSH is reachable as vegpu, but the passwordless vegpuctl control user is missing or blocked. Reset/reseed this VM disk so cloud-init can create vegpuctl.")
            }
        }
        throw RuntimeError.message("Timed out waiting for vEGPU runtime SSH")
    }
    ...
    public func waitForGuestAgent(timeout: TimeInterval = 300) async throws {
        ...
        while Date().timeIntervalSince(started) < timeout {
            ...
            let elapsed = Date().timeIntervalSince(started)
            if elapsed - lastReport > 8 {
                progress.report(ProgressEvent(stage: "agent", message: "Waiting for vEGPU guest agent", detail: "\(Int(elapsed))s elapsed"))
                lastReport = elapsed
            }
            try await Task.sleep(nanoseconds: 2_000_000_000)
        }
        throw RuntimeError.message("Timed out waiting for vEGPU guest agent sudo access")
    }
