...
public let guestShareRoot = "/mnt/vegpu-share"
public let linuxHomeExportPath = "/home/vegpu"
public let defaultLinuxHomeMountPath = "/Volumes/vEGPU/Home"
...
public final class NFSShareService: @unchecked Sendable {
    ...
    private let exportBegin = "# BEGIN vEGPU NFS SHARE"
    private let exportEnd = "# END vEGPU NFS SHARE"
    private let configBegin = "# BEGIN vEGPU NFS CONFIG"
    private let configEnd = "# END vEGPU NFS CONFIG"
    private let autoMasterBegin = "# BEGIN vEGPU LINUX HOME AUTOFS"
    private let autoMasterEnd = "# END vEGPU LINUX HOME AUTOFS"
    private let autoMapBegin = "# BEGIN vEGPU LINUX HOME MAP"
    private let autoMapEnd = "# END vEGPU LINUX HOME MAP"
    ...
    private func nfsSetupScript(exportLine: String) -> String {
        ...
        TMP="$(mktemp /tmp/vegpu-exports.XXXXXX)"
        CANDIDATE="$(mktemp /tmp/vegpu-exports-candidate.XXXXXX)"
        BACKUP="$(mktemp /tmp/vegpu-exports-backup.XXXXXX)"
        touch "$EXPORTS"
        cp "$EXPORTS" "$BACKUP"
        ...
        touch "$NFS_CONF"
        CONF_TMP="$(mktemp /tmp/vegpu-nfs-conf.XXXXXX)"
    ...
    private func ensureHostAutofsForLinuxHome(source: String, mountPath: String) async throws {
        ...
        _ = try await runner.runAdminScript(hostAutofsSetupScript(root: autofs.root, mapLine: mapLine), prompt: [
            "vEGPU needs your Mac password once to mount the Linux home folder",
    ...
    private func hostAutofsIsReady(root: String, mapLine: String) -> Bool {
        ...
        guard let master = try? String(contentsOfFile: "/etc/auto_master"),
              let map = try? String(contentsOfFile: "/etc/auto_vegpu") else {
            return false
        }
    ...
    private func hostAutofsSetupScript(root: String, mapLine: String) -> String {
        ...
        touch "$MASTER" "$MAP"
        TMP="$(mktemp /tmp/vegpu-auto-master.XXXXXX)"
        ...
        cat >> "$TMP" <<EOF
        $MASTER_BEGIN
        $ROOT auto_vegpu -nosuid
        $MASTER_END
        EOF
        install -m 0644 "$TMP" "$MASTER"
        rm -f "$TMP"
        mkdir -p "$ROOT"
        TMP="$(mktemp /tmp/vegpu-auto-map.XXXXXX)"
    ...
    private func repairHostLinuxHomeMount(source: String, mountPath: String) async throws {
        ...
        """
        _ = try await runner.runAdminScript(script, prompt: "vEGPU needs your Mac password to repair the Linux home NFS mount.")
    }
