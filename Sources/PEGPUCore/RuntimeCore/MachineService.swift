...
public final class MachineService: @unchecked Sendable {
    ...
    public func repairRunningMachine(reason: String) async throws {
        ...
        if synced {
            progress.report(ProgressEvent(stage: "ready", message: "PEGPU runtime is ready", level: .success))
        }
    }
    ...
    private func startMachineInner() async throws {
        ...
        case .gui:
            ...
            guard let angleFrameworks = AngleRuntime.frameworkDirectory(root: paths.root) else {
                throw RuntimeError.message("Missing app-side ANGLE runtime frameworks. Build the display runtime artifact and bundle EGL.framework and GLESv2.framework into PEGPU.app.")
            }
    ...
    private func repairRuntimeDependencies(shareRoot: String, reason: String) async throws {
        ...
        if config.launchMode == .gui {
            try await configureGuestAudioRTP()
            let micEnabled = ProcessInfo.processInfo.environment["PEGPU_AUDIO_MIC"] == "1"
            let bufferMs = Int(ProcessInfo.processInfo.environment["PEGPU_AUDIO_BUFFER_MS"] ?? "") ?? 20
            do {
            ...
            let command = [
                ...
                "/usr/local/sbin/pegpu-gui-ensure.sh"
            ].joined(separator: " ")
