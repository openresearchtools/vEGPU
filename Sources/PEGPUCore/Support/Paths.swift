...
public struct AppPaths: Sendable {
    ...
    public init(root: URL = AppPaths.discoverRoot(), environment: [String: String] = ProcessInfo.processInfo.environment) {
        self.root = root
        let dataRoot = environment["PEGPU_APP_DATA_DIR"].flatMap { $0.isEmpty ? nil : URL(fileURLWithPath: ($0 as NSString).expandingTildeInPath) }
            ?? FileManager.default.homeDirectoryForCurrentUser
                .appendingPathComponent("Library/Application Support/PEGPU/Machine", isDirectory: true)
