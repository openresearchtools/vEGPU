...
final class AppUpdateService: ObservableObject {
    enum Channel: String, CaseIterable {
        ...
        var manifestURL: URL {
            switch self {
            case .stable:
                return URL(string: "https://raw.githubusercontent.com/openresearchtools/vEGPU/main/releases/releases-manifest.json")!
            case .prerelease:
                return URL(string: "https://raw.githubusercontent.com/openresearchtools/vEGPU/main/releases/pre-releases-manifest.json")!
            }
        }
    }
    ...
    init(defaults: UserDefaults = .standard, session: URLSession = .shared) {
        ...
        self.updatesDir = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/vEGPU/Updates", isDirectory: true)
    }
    ...
    func checkForUpdates(silent: Bool = false) async {
        ...
        do {
            ...
            if let update {
                ...
                statusText = "vEGPU is up to date"
            }
        } catch {
            if !silent {
    ...
    func refreshAndReturnAvailableUpdate() async throws -> Entry? {
        ...
        let update = newestAvailableUpdate(from: manifest)
        availableUpdate = update
        statusText = update.map { "Update available: v\($0.version)" } ?? "vEGPU is up to date"
        return update
    }
    ...
    private func fetchManifest() async throws -> Manifest {
        ...
        request.setValue("vEGPU/\(Self.currentVersion)", forHTTPHeaderField: "User-Agent")
    ...
    private func safePackageName(_ name: String) -> String {
        let clean = name
            .split(separator: "/")
            .last
            .map(String.init) ?? "vEGPU-update.pkg"
        if clean.hasSuffix(".pkg") {
            return clean
        }
        return "vEGPU-update.pkg"
    }
