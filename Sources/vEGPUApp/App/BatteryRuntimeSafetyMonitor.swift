...
final class BatteryRuntimeSafetyMonitor: NSObject {
    ...
    private func showPanel() {
        ...
        panel.title = "vEGPU Battery Safety"
    ...
    private func updateCountdownLabels() {
        ...
        if shutdownInProgress {
            titleLabel?.stringValue = "Low battery: shutting down vEGPU VM"
            messageLabel?.stringValue = "vEGPU is stopping the VM through the normal guest and QEMU paths. If the VM does not stop cleanly, QEMU will be force killed."
        ...
        messageLabel?.stringValue = "Battery is \(percentText) and the Mac is running on battery. The vEGPU VM will shut down automatically to prevent full battery drain and PCIe sleep/panic risk."
    ...
    private func finishEmergencyShutdown(message: String, failed: Bool) {
        ...
        countdownLabel?.stringValue = failed ? "Check vEGPU" : "Sleep Guard Off"
        detailLabel?.stringValue = failed ? "vEGPU could not complete the emergency shutdown sequence. Plug in power now and check the runtime." : "The VM is stopped and Mac sleep prevention has been turned off."
