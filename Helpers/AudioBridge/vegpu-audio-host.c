...
static void usage(FILE *out) {
    fprintf(out,
        "usage: vegpu-audio-host --vm <ip> [--listen <port>] [--send <port>] [--mic] [--buffer-ms <ms>]\n"
        "\n"
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "vEGPU audio bridge running: vm=%s listen=%u send=%u mic=%s buffer_ms=%d\n",
            bridge.config.vm_host,
    ...
    fprintf(stderr, "vEGPU audio bridge stopped: underruns=%llu bad_packets=%llu\n",
